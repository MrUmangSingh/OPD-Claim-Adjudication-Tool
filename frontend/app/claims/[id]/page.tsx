...
export default function ClaimDetailPage() {
  ...
  const needsReview = claim.status === "manual_review";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          {decision && (
            <Card className={`border-2 ${decisionColors[decision.decision].replace("bg-", "border-").replace("text-", "").replace(/\s.*/, "")}`}>
              ...
              <CardContent className="space-y-4">
                ...
                {decision.notes && (
                  <p className="text-sm text-gray-600 bg-gray-50 rounded-md p-3">{decision.notes}</p>
                )}
          ...
          {(needsReview || claim.status === "decided") && (
